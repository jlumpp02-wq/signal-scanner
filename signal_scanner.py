...
Runs on GitHub Actions. Sends Discord + Telegram alerts.
...
def get_global_data():
    ...
    if data and "data" in data:
        ...
        return {"btc_dominance": btc_dom, "stablecoin_dominance": stable_dom, "total_market_cap": total_mcap}
...
def get_token_prices():
    ...
    data = fetch_json(f"https://api.coingecko.com/api/v3/simple/price?ids={ids}&vs_currencies=usd&include_24hr_change=true")
    prices = {}
    if data:
        for cg_id, symbol in TOKENS.items():
            if cg_id in data:
                prices[symbol] = {"price": data[cg_id]["usd"], "change_24h": round(data[cg_id].get("usd_24h_change", 0), 2)}
    return prices
...
def compute_market_grade(fg, social_sent, btc_dom, stable_dom):
    ...
    if asymmetry_score >= 80: grade, multiplier = "A+", 1.3
    elif asymmetry_score >= 65: grade, multiplier = "A", 1.2
    elif asymmetry_score >= 50: grade, multiplier = "B", 1.0
    elif asymmetry_score >= 35: grade, multiplier = "C", 0.85
    else: grade, multiplier = "D", 0.7
    return {"asymmetry_score": asymmetry_score, "grade": grade, "multiplier": multiplier, "regime": regime, "components": {"fear_greed": a, "social": b, "btc_dom": c, "stable_dom": d, "regime": e_score}}
...
def compute_token_score(rsi, obv_signal, multiplier):
    ...
    return {"rsi_score": rsi_s, "obv_score": obv_s, "raw_score": round(raw), "composite_score": composite}
...
def recommend_frequency(token_scores):
    ...
    if max_score >= 80: return "HOURLY", "High-conviction asymmetry detected"
    if max_score >= 65: return "4-HOUR", "Signals converging"
    if max_score >= 50: return "6-HOUR", "Moderate setups forming"
...
def send_discord_alert(market, btc, token_results, grade_data, freq):
    if not DISCORD_WEBHOOK:
        print("No DISCORD_WEBHOOK set, skipping Discord")
        return
    actionable = [t for t in token_results if t["composite_score"] >= ALERT_THRESHOLD]
    grade_emoji = {"A+": "G", "A": "G", "B": "Y", "C": "O", "D": "R"}.get(grade_data["grade"], "?")
    market_text = (
    ...
    embed = {
        "title": f"SignalScanner Grade {grade_data['grade']} ({grade_data['asymmetry_score']}/100) {grade_data['multiplier']}x",
...
# --- Telegram Alert ---
def send_telegram_alert(market, btc, token_results, grade_data, freq):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        print("No TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID set, skipping Telegram")
        return
    actionable = [t for t in token_results if t["composite_score"] >= ALERT_THRESHOLD]
    lines = [
        f"SIGNALSCANNER - Grade {grade_data['grade']} ({grade_data['asymmetry_score']}/100) - {grade_data['multiplier']}x",
        "",
        market.get("summary", ""),
        "",
        "MARKET CONDITIONS",
    ...
    if actionable:
        lines.append("ASYMMETRIC OPPORTUNITIES")
        for t in actionable:
            lines.append(f"  >> {t['symbol']} - Score {t['composite_score']}, RSI {t['rsi']}, {t['obv_signal']}")
        lines.append("")
    lines.append(f"TOKEN SIGNALS ({len(actionable)} actionable)")
    for t in token_results[:9]:
        label = signal_label(t["composite_score"])
        price_str = f"${t['price']:.4g}" if t["price"] else "?"
        lines.append(f"  {t['symbol']} {price_str} ({t['change_24h']:+.1f}%) | RSI {t['rsi'] or '?'} | {t['obv_signal']} | {t['composite_score']} {label}")
    lines.extend(["", f"Scan: {freq[0]} - {freq[1]}"])
    text = "\n".join(lines)
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = json.dumps({"chat_id": TELEGRAM_CHAT_ID, "text": text, "disable_web_page_preview": True}).encode()
...
def run_scan():
    ...
    social_sentiment = max(20, min(90, 100 - fg["value"]))

    grade_data = compute_market_grade(fg["value"], social_sentiment, global_data["btc_dominance"], global_data["stablecoin_dominance"])
    ...
    for cg_id, symbol in TOKENS.items():
        ...
        result = {"symbol": symbol, "price": price_data["price"], "change_24h": price_data["change_24h"], "rsi": rsi, "obv_signal": obv_signal, **scores}
    ...
    send_telegram_alert(market_data, btc, token_results, grade_data, freq)

    scan_result = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "market": {"fear_greed": fg, "btc": btc, "btc_dominance": global_data["btc_dominance"], "stablecoin_dominance": global_data["stablecoin_dominance"], "grade": grade_data, "regime": grade_data["regime"], "summary": summary},
